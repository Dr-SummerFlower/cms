...
export async function listConcerts(q?: ConcertQuery): Promise<Paginated<Concert>> {
...
export async function createConcert(dto: CreateConcertDto, poster: File | undefined): Promise<Concert> {
...
export async function updateConcert(id: string, dto: UpdateConcertDto): Promise<Concert> {
  const raw = await patchJson<ConcertRaw, UpdateConcertDto>(`/concerts/${id}`, dto);
  return toConcert(raw);
}

export async function updateConcertPoster(id: string, poster: File): Promise<Concert> {
  const form = new FormData();
