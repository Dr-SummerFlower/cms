    ...
    try {
      ...
      if (approved) {
        ...
        try {
          ...
          this.logger.error(
            `发送退票拒绝邮件失败 [${emailError instanceof Error ? emailError.constructor.name : typeof emailError}]: ${emailError instanceof Error ? emailError.message : String(emailError)}`,
      ...
      try {
        ...
        this.logger.error(
          `更新退票申请过期时间时发生错误 [${finallyError instanceof Error ? finallyError.constructor.name : typeof finallyError}]: ${finallyError instanceof Error ? finallyError.message : String(finallyError)}`,
          finallyError instanceof Error ? finallyError.stack : undefined,
        );
      }
    }
  }
