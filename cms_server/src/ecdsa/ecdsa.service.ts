...
export class EcdsaService {
  ...
  generateKeyPair(): EcdsaKeyPair {
    try {
      ...
      this.logger.error(
        `ECDSA 密钥对生成失败 [${error instanceof Error ? error.constructor.name : typeof error}]: ${error instanceof Error ? error.message : String(error)}`,
  ...
  sign(data: string, privateKey: string): EcdsaSignature {
    try {
      ...
      this.logger.error(
        `ECDSA 签名失败，请检查私钥格式是否正确 [${error instanceof Error ? error.constructor.name : typeof error}]: ${error instanceof Error ? error.message : String(error)}`,
        error instanceof Error ? error.stack : undefined,
      );
